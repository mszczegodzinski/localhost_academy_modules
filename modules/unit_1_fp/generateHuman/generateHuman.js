...
function generateName(minLength = 3, maxLength = 10) {
    ...
    if (minLength > maxLength) {
        throw new Error('min name or surname length have to be less than or equal to max length')
    } 
...
function generatePhoneNr() {
    ...
    const phoneNr = Math.floor(Math.random() * (maxPhoneNr - minPhoneNr + 1)) + minPhoneNr;
    //zmienic na string
    return phoneNr;
}
