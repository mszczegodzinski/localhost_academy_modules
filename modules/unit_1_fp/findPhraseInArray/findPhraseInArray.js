...
const inputData = ['stwórz', 'sobie' ,'tutaj', 'więcej', 'wyrazów', 'wyrazSzosty', 'wyrazSiodmy', 'wyrazOsmy', 'wyrazDziewiaty', 'wyrazDziesiaty', 'wyrazJedenasty', 'wyrazDwunasty','wyrazTrzynasty', 'wyrazCzternasty', 'wyrazPietnasty']
...
function findPhraseInArray(array, phrase){
    ...
    if(array.length < 1){
        throw new Error(`Incorrect array length. Array should have at least ${minArrLength} words`)
    }
    if(typeof phrase !== 'string'){
        throw new Error("Incorrect searching phrase. Phrase can be only a string")
    }
    if(phrase.length < 1){
        throw new Error(`Incorrect phrase length. Phrase should have at least ${minPhraseLength} letters`)
    }
    const phraseRegex = new RegExp(phrase, 'gi');
    // const resultArr = [];
    // array.forEach((word, i) => {
    //     const foundedPhrase = word.match(phraseRegex) ? word : null;
    //     if(foundedPhrase !== null) {
    //         resultArr.push([i, foundedPhrase]);
    //     }
    // })
    // .reduce(()=>{},[])
    const resultArr = array.reduce((total, currentWord) => {
